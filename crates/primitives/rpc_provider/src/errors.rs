...
impl From<StarknetWsApiError> for StarknetRpcApiError {
    fn from(err: StarknetWsApiError) -> Self {
        match err {
            StarknetWsApiError::TooManyBlocksBack => {
                StarknetRpcApiError::ErrUnexpectedError { data: "Cannot go back more than 1024 blocks".to_string() }
            }
            StarknetWsApiError::NoBlocks => StarknetRpcApiError::NoBlocks,
            StarknetWsApiError::BlockNotFound => StarknetRpcApiError::BlockNotFound,
            StarknetWsApiError::Pending => StarknetRpcApiError::ErrUnexpectedError {
                data: "The pending block is not supported on this method call".to_string(),
            },
            StarknetWsApiError::Internal => StarknetRpcApiError::InternalServerError,
        }
    }
}
