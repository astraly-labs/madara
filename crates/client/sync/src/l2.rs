...
async fn l2_pending_block_task(
    ...
    while wait_or_graceful_shutdown(interval.tick()).await.is_some() {
        ...
        let block = block_import.pre_validate_pending(block, context.clone()).await?;
        block_import.verify_apply_pending(block, context.clone()).await?;
    }
...
pub async fn sync(
    ...
    let context = ValidationContext { trust_transaction_hashes: false, chain_id };
